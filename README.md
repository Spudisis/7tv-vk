...
Два тега прямо в тексте сообщения:

- `[spoiler]текст[/spoiler]` — текст между тегами уходит под блюр.
- `[spoiler/]` — под блюром всё содержимое сообщения: и текст, и фотографии
  с другими вложениями. Имя отправителя, время и аватар остаются видимыми.
  Ставится в любом месте текста.
- `spoiler` или `спойлер` — то же самое, что `[spoiler/]`, но без скобок.
  Работает, только если в сообщении нет ничего, кроме этого слова: одно
  слово к фотографии — и она под блюром.

Клик по замазанному месту открывает его насовсем — до того, как ВК
перерисует сообщение; клик по закрытой картинке открывает сообщение
целиком. Сами теги в сообщении не показываются, эмоуты внутри спойлера
работают как обычно. Регистр в теге не важен, незакрытый `[spoiler]`
прячет текст до конца сообщения.

Выделив текст в поле ввода, тег можно поставить кнопкой с глазом — она
встаёт первой в панели форматирования ВК.

На сервере ВК остаётся обычный текст с тегами: у кого расширения нет, тот
увидит и теги, и то, что под ними.
...
- `emotes.js` — чтение настроек и сборка эмоутов; общая для страницы и попапа.
- `content.js` / `content.css` — замена слов на эмоуты и спойлеры.
- `autocomplete.js` — автозаполнение эмоутов при наборе (Twitch-стиль).
- `composer.js` — кнопка спойлера в панели форматирования ВК.
...
- `tests/` + `run-tests.sh` — стенды в headless Chrome (в архив расширения не входят).
