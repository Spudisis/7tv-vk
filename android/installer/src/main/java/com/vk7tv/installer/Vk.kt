...
object Vk {
    ...
    // Голый «vk» отсюда убран намеренно: он ловил всё подряд — например RuStore
    // (ru.vk.store) и другие приложения VK, которые клиентами не являются.
    // Оба официальных клиента и так перечислены в KNOWN, а сторонние узнаём по
    // характерным именам. Обскурные клиенты пользователь добавит через поиск.
    private val HINTS = listOf("вконтакте", "vkontakte", "sova", "kate")
