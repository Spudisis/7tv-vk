    ...
    fun show(anchor: View) {
        ...
        if (Startup.safeMode) {
            ...
            root.addView(
                note(
                    ctx,
                    "Модуль выключил эмоуты и картинки: приложение не пережило прошлый " +
                        "запуск. Так оно хотя бы открывается. " +
                        if (Startup.willReturn) {
                            "Само включится обратно, когда закроешь и откроешь ВК ещё " +
                                "раз; кнопка — чтобы не ждать."
                        } else {
                            "Вернуться пробовали, и запуск снова не пережили, — дальше " +
                                "только этой кнопкой. Если падать начнёт снова, напиши " +
                                "нам и приложи скрин."
                        },
                ),
            )
        ...
        root.addView(
            note(
                ...
                "По умолчанию коды превращаются в эмоуты только в переписке: " +
                    "в сообщениях диалога и в превью списка чатов. Шапка чата, " +
                    "вкладки внизу и счётчики непрочитанных остаются как есть. " +
                    "Включи — эмоуты будут везде: в ленте, комментариях, названиях " +
                    "групп и в счётчиках тоже.",
            ),
        )
