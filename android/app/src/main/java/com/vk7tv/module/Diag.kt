 ...
 * всегда, тостом — только когда о нём спросили. Исключение — [warn]: что
 * 7tv.io недоступен и нужен VPN, человек должен узнать сразу, иначе пустой
 * пикер выглядит как поломка модуля.
 */
object Diag {
    ...
    /**
     * Показать всегда, мимо галки. Только про внешние помехи: 7tv.io не
     * ответил, наборы не приехали. Про сам модуль так писать не надо — для
     * этого есть [note] и [alert].
     */
    fun warn(msg: String) {
        L.i(msg)
        val c = ctx ?: return
