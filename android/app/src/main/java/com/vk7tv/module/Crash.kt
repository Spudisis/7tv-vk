    ...
    private class Guard(private val prev: Thread.UncaughtExceptionHandler?) :
        ...
        override fun uncaughtException(t: Thread, e: Throwable) {
            L.safe("запись вылета") {
                ...
                XposedBridge.log(e)
                // В журнал строкой: файл вылета показывает только свои падения,
                // а по журналу видно и чужие — вместе с тем, что модуль делал
                // перед этим
                Journal.add("НЕПОЙМАННЫЙ вылет в потоке «${t.name}»: $e")
