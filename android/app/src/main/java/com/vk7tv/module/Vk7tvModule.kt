...
class Vk7tvModule : IXposedHookLoadPackage {
    ...
    override fun handleLoadPackage(lpp: XC_LoadPackage.LoadPackageParam) {
        ...
        // Битая копия оригинального APK роняет клиент при загрузке его
        // классов — раньше Application.onCreate, откуда поднимается всё
        // остальное. Поэтому проверка стоит здесь, а не в Boot.ensure: туда
        // при таком падении исполнение не доходит. Журнал подключаем тем же
        // ранним порядком, иначе строчка о найденной поломке не переживёт
        // гибель процесса и причина падения снова останется невидимой.
        L.safe("ранний журнал") {
            lpp.appInfo?.dataDir?.let { Journal.attach(File(it, "files/vk7tv").apply { mkdirs() }) }
        }
        L.safe("проверка копии оригинального APK") { LsPatch.checkOrigin(lpp.appInfo) }
            ...
            XposedHelpers.findAndHookMethod(
                ...
                object : XC_MethodHook() {
                    override fun afterHookedMethod(param: MethodHookParam) {
                        L.safe("старт") {
                            val app = param.thisObject as? Context ?: return@safe
                            // Процессов у клиента несколько (:push и прочие).
                            // Интерфейса они не показывают, живут секундами, а
                            // модуль в них тянул наборы с 7tv.io и писал конфиг.
                            // Такой процесс поднимет модуль сам, если дойдёт до
                            // отрисовки текста: хук setText зовёт Boot.ensure.
                            if (Boot.headless(app)) {
                                L.i("процесс ${Application.getProcessName()} без интерфейса — не поднимаемся")
                                return@safe
                            }
                            Boot.ensure(app)
                        }
...
object Boot {
    ...
    /**
     * Процесс без интерфейса: имя не совпадает с именем пакета. У ВК это
     * :push и подобные — они запускаются на каждое уведомление и живут
     * секундами. Определить нечем — считаем, что интерфейс есть: пропустить
     * эмоуты хуже, чем лишний раз подняться.
     */
    fun headless(ctx: Context): Boolean {
        val name = L.safe("имя процесса") { Application.getProcessName() } ?: return false
    ...
    fun ensure(ctx: Context?) {
        ...
        synchronized(this) {
            ...
            // Аварийный режим: если модуль не пережил прошлый запуск, не трогаем
            // ни текст, ни картинки — только даём клиенту открыться. Нативный
            // вылет (декодер картинок и т.п.) не ловится, поэтому единственная
            // защита — вообще не запускать рискованную работу.
            Startup.attach(data, app)
            if (Startup.safeMode) {
                // всегда, мимо галки: эмоуты пропали целиком, и человек должен
                // понимать, почему, — иначе это выглядит как сломанный модуль
                Diag.warn(
                    "аварийный режим: эмоуты выключены — клиент не пережил прошлый запуск. " +
                        if (Startup.willReturn) {
                            "Модуль включит их сам, когда закроешь и откроешь ВК ещё раз."
                        } else {
                            "Включить обратно можно в настройках (шестерёнка в пикере " +
                                "или долгий тап по кнопке 7VK)."
                        },
                )
