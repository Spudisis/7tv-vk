 ...
 * OnItemTouchListener). Туда события приходят РАНЬШЕ, чем к нам, поэтому наше
 * «съел» его не останавливает: поверх поповера открывалось меню сообщения.
 * Лечится штатным requestDisallowInterceptTouchEvent — им ребёнок и говорит
 * родителям, что жест занят.
 *
 * Что при этом не ломается: прокрутка списка (как только палец поехал, запрет
 * снимаем, и список забирает жест сам) и любые касания мимо слова — там мы
 * возвращаем false, и ВК даже не узнает, что мы здесь были.
 *
 * Что меняется: на самом слове нет меню сообщения ВК. Держать его там не
 * вышло — слово в строке маленькое, палец на нём задерживается, и обычный тап
 * регулярно переваливал за порог долгого нажатия. Пусть лучше нажатие на слово
 * всегда делает ровно одно и то же; меню сообщения остаётся на всей остальной
 * площади пузыря.
 */
object Taps {
    ...
    private fun grab(tv: TextView, e: MotionEvent): Boolean {
        if (e.actionMasked == MotionEvent.ACTION_DOWN) {
            ...
            // У вьюхи есть movementMethod — значит спаны в ней нажимает сам ВК,
            // тем же кодом, что ссылки и упоминания. Не лезем: он про своё меню
            // сообщения знает, а мы нет, и вдвоём мы открывали два окна разом.
            if (tv.movementMethod != null) return false
            ...
            moved = false
            // «Жест мой, руки прочь» — вверх по всем родителям. Без этого ВК
            // ловит долгое нажатие сам, у себя в onInterceptTouchEvent: туда
            // касание приходит РАНЬШЕ нас, и то, что мы его потом съели, ему
            // безразлично — поверх нашего поповера открывалось меню сообщения.
            // Это же штатный способ сказать RecyclerView, что гостя больше нет:
            // он пробрасывает отказ своим OnItemTouchListener, и те гасят свои
        ...
        when (e.actionMasked) {
            MotionEvent.ACTION_MOVE ->
                // палец поехал — это прокрутка, а не тап: возвращаем список
                // себе, иначе он не сможет прокрутиться, раз мы запретили
                // родителям перехват. Перехватит на следующем же движении
                // и пришлёт нам ACTION_CANCEL.
            ...
            MotionEvent.ACTION_UP -> {
                val w = word
                val at = box
                // Держал долго — считаем, что ВК всё-таки успел показать своё
                // меню (запрет перехвата помогает не на всякой разметке).
                // Тогда молчим: одно чужое меню лучше, чем два сразу.
                val quick = e.eventTime - e.downTime < ViewConfiguration.getLongPressTimeout()
                val tap = !moved && quick
                release()
    ...
    private fun rectOf(tv: TextView, s: SuggestSpan): Rect? = SuggestUi.rectOf(tv, s)
