...
/**
 * Служебные подписи ВК — время сообщения, «(ред.)», дата, счётчики — не текст
 * пользователя, и трогать их нельзя: в наборах попадаются эмоуты с именами
 * вроде «20:00» и «67», и без этой проверки время сообщения или число
 * непрочитанных превращается в картинку.
...
object Service {
    ...
    // «Сообщения», «N участника» в шапке. Проверяем и родителей: у самого
    // текста бейджа id часто нет, он есть у контейнера.
    ...
    fun isServiceView(tv: TextView): Boolean {
        if (hasToken(tv, TOKENS)) return true
        var node: View? = tv
        var depth = 0
