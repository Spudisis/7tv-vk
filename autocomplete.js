...
(() => {
  ...
  function onInput(e) {
    ...
    for (const [name, v] of st.emoteMap) {
      ...
      // пока не начнёшь набирать «_»: голое имя у кода теперь есть всегда
      // (при коллизии — набор, первый по алфавиту), а конкретный вариант
      // виден, как только допишешь постфикс.
      if (v.a && !word.includes('_') && st.emoteMap.has(v.a)) continue;
