...
(() => {
  ...
  // слово целиком из букв/цифр/подчёркиваний/дефисов, с разделителем внутри
  // и не короче, чем «xx_yyy». Хвостовые и подряд идущие «_» допускаем:
  // ник стримера бывает и таким (peeb_iluci____ → ник iluci____). Дефис —
  // ради имён эмоутов вроде «-F» (-F_simelly → эмоут «-F», ник simelly);
  // границу «эмоут|ник» разбирает splitCandidates в фоне, а API проверит.
